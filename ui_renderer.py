...
def render_facts(cv, ctx):
    """The flashcard back side: today, in plain facts. Reached by
    tapping the display; tapping again (or 60s) flips back."""

    _center(cv, 45, "TODAY, IN DETAIL", 18)
    date_str = "%s, %s %d" % (ctx["weekday_name"], ctx["month_name"],
                              ctx["day"])
    _center(cv, 95, date_str, 24)
    cv.line(W // 4, 150, 3 * W // 4, 150)
    ...
    for label, value in rows:
        cv.text(70, y + 4, label.upper(), 18)
        vw = cv.text_width(value, 24)
        cv.text(W - 70 - vw, y, value, 24)
        y += 56

    doy, total = _day_of_year(ctx)
    _center(cv, y + 30, "Day %d of %d" % (doy, total), 18)
    _center(cv, 910, "TAP TO FLIP BACK", 18)
